...
The system SHALL show a second animation phase immediately after the first completion GIF phase and SHALL source that second GIF from `assets/star-gifs/<stars>-stars.gif` using the same achieved star count.
...
- **THEN** the second animation uses `assets/star-gifs/3-stars.gif`
...
- **WHEN** completion celebration requirements tests are executed
- **THEN** tests verify that a star-bucket completion GIF is shown first and the corresponding `assets/star-gifs/<stars>-stars.gif` is shown second
