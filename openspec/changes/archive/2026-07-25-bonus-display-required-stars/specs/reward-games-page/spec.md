...
#### Scenario: Implemented but not yet unlocked reward game appears locked
- **WHEN** the reward games page renders a reward game entry that has an available game route but whose unlock criteria are not satisfied
- **THEN** the entry shows a visible locked state and is unavailable for activation

#### Scenario: Locked reward game displays total-star progress text
- **WHEN** the reward games page renders a locked reward game entry
- **THEN** the entry displays `<current won stars>/<required stars>` using current total earned stars and the entry's configured total-star unlock threshold

#### Scenario: Implemented and unlocked reward game entry is launchable
- **WHEN** the reward games page renders a reward game entry that has an available game route and satisfied unlock criteria
- **THEN** the entry is presented without a lock overlay and is available for activation

#### Scenario: Activating an implemented and unlocked reward game navigates to its route
- **WHEN** a user activates a reward game entry that has an available game route and satisfied unlock criteria
...
#### Scenario: Unlock-gated locked state is tested
- **WHEN** reward games page requirements tests are executed
- **THEN** a test verifies that a reward game entry with an available route but unsatisfied unlock criteria is presented with a visible locked state and is not selectable

#### Scenario: Locked progress text is tested
- **WHEN** reward games page requirements tests are executed
- **THEN** a test verifies that locked entries display progress text formatted as `<current won stars>/<required stars>` with values sourced from earned total stars and configured total-star threshold

#### Scenario: Launchable entry presentation is tested
- **WHEN** reward games page requirements tests are executed
- **THEN** a test verifies that a reward game entry with an available route and satisfied unlock criteria is rendered without a lock overlay and is available for activation
