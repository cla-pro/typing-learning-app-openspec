...
### Requirement: Shuffle button label is localized
The system SHALL display the shuffle button label using the active UI language via the existing translation mechanism. The translation key `exercise.shuffle` SHALL be defined in all supported locale files.

#### Scenario: Shuffle button label appears in French
- **WHEN** the active locale is `fr-ch` and the shuffle button is visible
- **THEN** the button label is `Mélanger`

#### Scenario: Shuffle button label appears in German
- **WHEN** the active locale is `de-ch` and the shuffle button is visible
- **THEN** the button label is `Mischen`

#### Scenario: Shuffle button label appears in English
- **WHEN** the active locale is `en-us` and the shuffle button is visible
- **THEN** the button label is `Shuffle`
