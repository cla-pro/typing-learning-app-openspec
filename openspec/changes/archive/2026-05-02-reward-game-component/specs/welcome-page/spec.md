...
The system SHALL display a welcome page with navigation to exercise pages when a user navigates to the root URL, SHALL obtain the currently chosen keyboard layout, SHALL source grouped navigation from ordered exercise categories returned by `ExerciseConfigService` for that chosen layout, SHALL render its in-scope welcome-page UI strings from the active UI language instead of hardcoded English text, and SHALL display a reward-games entry action on the welcome page only after the user has earned at least one star across all exercises.
...
#### Scenario: Reward games entry action is hidden before any stars are earned
- **WHEN** the welcome page loads for a user who has earned zero total stars
- **THEN** no reward games entry action is displayed

#### Scenario: Reward games entry action is displayed after a star is earned
- **WHEN** the welcome page loads for a user who has earned at least one total star
- **THEN** a reward games entry action using the `🎮` icon is displayed with its visible text or accessible label resolved from the active UI language
...
The system SHALL include automated tests for welcome-page exercise-link integration with `ExerciseConfigService`, including grouped data and ordering behavior, SHALL verify chosen-layout propagation, SHALL verify gear-button rendering and navigation behavior, and SHALL verify reward-games entry visibility behavior based on the user's earned stars.
...
#### Scenario: Reward-games entry visibility is tested
- **WHEN** welcome component requirements tests are executed
- **THEN** tests verify that the reward-games entry is hidden for zero total stars and rendered once at least one star has been earned